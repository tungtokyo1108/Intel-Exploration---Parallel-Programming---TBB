...
#ifndef INCLUDE_TBB_TBB_THREAD_H_
#define INCLUDE_TBB_TBB_THREAD_H_
...
template <class F, class X> struct thread_closure_1 : thread_closure_base {
	F function;
	X arg1;
	// Routine passed to Window's _beginthreadx by thread::internal_start() inside tbb,dll
	static __TBB_NATIVE_THREAD_ROUTINE start_routine(void* c) {
...
class tbb_thread_v3 {
  ...
  /*
  * Work around for a compiler bug: declaring the copy constructor as public
  * enables use of the moving constructor
  * The difinition is not provided in order to prohibit copying
  */
public:
  #endif
...
public:
  ...
  #endif
  class id;
  /* Constructs a thread object that dose not represent a thread of execution */
...
tbb_thread_v3(tbb_thread_v3&& x) __TBB_NOEXCEPT(true) : my_handle(x.my_handle)
#if _WIN32||_WIN64
...
// When detach() return, *this no longer represents the possibly continuing thread of execution
...
#endif /* INCLUDE_TBB_TBB_THREAD_H_ */
