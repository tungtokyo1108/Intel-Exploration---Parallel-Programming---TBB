...
iterator grow_by(size_type delta, const_reference t) {
	return iterator(*this, delta ? internal_grow_by(delta, sizeof(T), &initialize_array_by,
			static_cast<const void*>(&t)) : my_early_size.load());
}
...
iterator grow_by(I first, I last) {
	...
	return iterator(*this, delta ? internal_grow_by(delta, sizeof(T), &copy_range<I>,
			static_cast<const void*>(&first)) : my_early_size.load());
}
 ...
 *
 * The new elements are default constructed.
 * Blocks until all elements in range [0,..,n) are allocated.
 * Can return while other elements are being constructed by other threads.
...
/*
 * Analogous to grow_to_at_least(size_type n) with exception that the new elements
 * are initialized by copying of t instead of default construction
 */
...
#if __TBB_CPP11_VARIADIC_TEMPLATE_PRESENT
 ...
 * while growing the vector, as long as the calling thread has checked that index < size()
 */
...
size_type size() const {
	size_type sz = my_early_size;
	size_type cp = internal_capacity();
	return cp < sz ? cp : sz;
}
...
void assign(size_type n, const_reference t) {
	clear();
	internal_resize(n,sizeof(T),max_size(),static_cast<const void*>(&t),&destroy_array,&internal_array_by);
}
template <typename I>
...
~concurrent_vector() {
	segment_t *table = my_segment.load<relaxed>();
	internal_free_segements(table, internal_clear(&destroy_array),my_first_block.load<relaxed>());
}
...
void internal_free_segments(segment_t table[], segment_index_t k, segement_index_t first_block);
...
static void __TBB_EXPORTED_FUNC initialize_array_by(void* begin, const void*, size_type n);
...
~internal_loop_guide() {
	if (i < n) {
		internal::handle_unconstructed_elements(array+1, n-1);
	}
}
};
...
struct push_back_helper : internal::no_copy {
	...
	push_back_helper(concurrent_vector &vector) :
		v(vector), g(static_cast<T*>(v.internal_push_back(sizeof(T),k)))
	{}
...
T& concurrent_vector<T,A>::internal_subscript(size_type index) const {
	...
	// The information about added elements via some form fo external sychronization
...
T& concurrent_vector<T,A>::internal_subscript_with_exceptions(size_type index) const {
	...
	size_type j = index;
	segment_index_t k = segment_base_index_of(j);
	if (my_segment.load<acquired>() == my_storage && k >= pointer_per_short_table)
		internal::throw_exception(internal::eid_segment_range_error);
	segment_value_t segment_value = my_segment[k].template load<relaxed>();
	enfore_segment_allocated(segment_value, internal::eid_index_range_error);
	return (segment_value.pointer<T>())[j];
}

template <typename T, class A> template <class I>
void concurrent_vector<T,A>::internal_assign_iterator(I first, I last) {
	...
	while (sz < n)
	{
		internal_loop_guide_loop(sz,my_segment[k].template load<relaxed>().template pointer<void>());
		loop.iterate(first);
		n -= sz;
...
template <typename T, class A>
inline void swap (const concurrent_vector<T,A>& a, const concurrent_vector<T,A>& b)
{
	a.swap(b);
}
}
